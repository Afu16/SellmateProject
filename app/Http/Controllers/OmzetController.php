...
public function komisi(Request $request)
{
    ...
    // Total komisi keseluruhan user (omzets sudah berupa komisi di seeder)
    $totalKomisi = Omzet::where('user_id', $userId)->sum('total_omzets');

    // Query transaksi sesuai rentang atau bulan
    ...
    $totalKomisiBulanIni = $komisiBulanIni->sum('total_omzets');

    // Aggregasi per minggu (1–7, 8–14, 15–21, 22–akhir)
    ...
    foreach ($komisiBulanIni as $o) {
        ...
        $weeks[$bucket] += (int) $o->total_omzets;
